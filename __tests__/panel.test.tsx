...
test('Editable Panel label uses an explicit edit control and Escape cancels without submitting', () => {
  const onLabelEdit = vi.fn();
  render(
    <ReqoreUIProvider>
      <ReqoreLayoutContent>
        <ReqorePanel label='Selectable panel title' onLabelEdit={onLabelEdit}>
          Panel
        </ReqorePanel>
      </ReqoreLayoutContent>
    </ReqoreUIProvider>
  ...
  fireEvent.click(screen.getByText('Selectable panel title'));
  expect(screen.queryByRole('textbox')).not.toBeInTheDocument();

  fireEvent.click(screen.getByRole('button', { name: 'Edit panel label' }));
  const input = screen.getByRole('textbox');
  fireEvent.change(input, { target: { value: 'Changed panel title' } });
  fireEvent.keyDown(input, { key: 'Escape' });

  expect(screen.queryByRole('textbox')).not.toBeInTheDocument();
  expect(screen.getByText('Selectable panel title')).toBeInTheDocument();
  expect(onLabelEdit).not.toHaveBeenCalled();
});

test('Editable Panel label commits on Enter and blur', () => {
  const onLabelEdit = vi.fn();
  render(
    <ReqoreUIProvider>
      <ReqoreLayoutContent>
  ...
  fireEvent.click(screen.getByRole('button', { name: 'Edit panel label' }));
  fireEvent.change(screen.getByRole('textbox'), { target: { value: 'Enter title' } });
  fireEvent.keyDown(screen.getByRole('textbox'), { key: 'Enter' });
  expect(onLabelEdit).toHaveBeenLastCalledWith('Enter title');

  fireEvent.click(screen.getByRole('button', { name: 'Edit panel label' }));
  fireEvent.change(screen.getByRole('textbox'), { target: { value: 'Blur title' } });
